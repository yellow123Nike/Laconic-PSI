...
namespace osuCrypto
{
        ...
        Channel& chl)
    {
        ...
        chl.recv(correction);
        auto iter = correction.begin();//返回一个迭代器指向第一个元素
        //01翻转
    ...
    //运行silent rot 输出b(messages).执行完后生成randomots a[i]=b[i][c[i]]
    void SilentOtExtSender::silentSend(
        span<std::array<block, 2>> messages,
        PRNG& prng,
        Channel& chl)
    {
        //运行silentcot,输入detla(prng.get),向量长度(messages.size),随机源，通信通道
        //输出存在如下关系a[i] = b[i] + c[i] * delta
